...
class Buildamic extends BuildamicBase
{
  ...
  {
    return new Fields($this->config("sets.main.sets.{$setHandle}.fields"), $this->field()->parent(), $this->field());
  }
  ...
  public function preload()
  {
    ...
    return [
      ...
      'sets' => collect($this->config('sets.main.sets') ? $this->config('sets.main.sets') : $this->config('sets'))->map(function ($set, $handle) use ($instance) {
        $fields = [
          'handle' => $handle,
          'display' => $set['display'],
          'fields' => [],
